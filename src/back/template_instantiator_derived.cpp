...
StructTemplateInstantiator::StackElement StructMemberTemplateInstantiator::placeholder( Placeholder ph )
{
	...
	switch( ph.id )
	{
		...
		case PLACEHOLDER::ENCODING_MEMBER_TYPE:
		{
...
void MemberTypeTemplateInstantiator::execBuiltinFunction( Stack& stack, PREDEFINED_FUNCTION fnID )
{
	switch ( fnID )
	{
		...
		case PREDEFINED_FUNCTION::SEQUENCE_SIZE:
		{
			assert( stack.size() >= 1 ); // TODO: it's a common check. Think about generalization
			auto arg = stack.begin() + stack.size() - 1;
			assert( arg->argtype == ARGTYPE::STRING ); // TODO: think about type convergence
			StackElement elem;
			elem.argtype = ARGTYPE::STRING;
			if(dataType->kind != DataType::KIND::SEQUENCE)
			{
				// TODO: report error: this function cannot be called in this context
				assert( 0 );
			}
			auto subst = dataType->mappingRepresentation->mappingAttrs.find( "_sequence_size" );
			assert( subst != dataType->mappingRepresentation->mappingAttrs.end() );
			assert( subst->second.kind == Variant::KIND::STRING );
			vector<LinePart> lineParts;
			LinePart lp;
			lp.type = PLACEHOLDER::VERBATIM;
			size_t sz = subst->second.stringValue.size();
			for ( size_t i=0; i<sz; ++i )
			{
				if ( subst->second.stringValue[i] != '%' )
				{
					lp.verbatim.push_back( subst->second.stringValue[i] );
				}
				else
				{
					if ( !( i + 1 < sz && subst->second.stringValue[i+1] == '1') )
					{
						lp.verbatim.push_back( subst->second.stringValue[i] );
						continue;
					}
					...
					for( auto& lpart:arg->lineParts )
						lineParts.push_back( lpart );
				}
			}
			if ( lp.verbatim.size() )
				lineParts.push_back( lp );
			lp.verbatim = TemplateInstantiator::resolveLinePartsToString( lineParts );
			elem.lineParts.push_back( lp );
			stack.pop_back();
			stack.push_back( std::move(elem) );
			break;
		}
		case PREDEFINED_FUNCTION::SERILAIZE_SEQUENCE_BEGIN:
		{
		...
		case PREDEFINED_FUNCTION::SERILAIZE_IS_SEQUENCE_END:
		{
			assert( stack.size() >= 2 ); // TODO: it's a common check. Think about generalization
			auto arg1 = stack.begin() + stack.size() - 2;
			auto arg2 = stack.begin() + stack.size() - 1;
		...
		case PREDEFINED_FUNCTION::SERILAIZE_SEQUENCE_NEXT:
		{
			assert( stack.size() >= 2 ); // TODO: it's a common check. Think about generalization
			auto arg1 = stack.begin() + stack.size() - 2;
			auto arg2 = stack.begin() + stack.size() - 1;
		...
		case PREDEFINED_FUNCTION::DESERILAIZE_SEQUENCE_ADD_NEXT:
		{
			assert( stack.size() >= 2 ); // TODO: it's a common check. Think about generalization
			auto arg1 = stack.begin() + stack.size() - 2;
			auto arg2 = stack.begin() + stack.size() - 1;
